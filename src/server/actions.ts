...
export default class Actions {
  ...
  constructor () {
    logger.info('Creating the faucet account');
  ...
  async sendTokens (address: string, amount: string): Promise<DripResponse> {
    ...
    try {
      ...
      const transfer = apiInstance.tx.balances.transfer
        ? apiInstance.tx.balances.transfer(address, dripAmount)
        : apiInstance.tx.balances.transferAllowDeath(address, dripAmount);
